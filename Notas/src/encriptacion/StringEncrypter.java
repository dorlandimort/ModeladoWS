...
public class StringEncrypter {
     ...
     */
    public static String encriptarCadena(String s) {
        try {
            Polybius p = new PolybiusImpl();
            String cadena = p.toPolybius(s);
            SecretKey desKey = KeyGenerator.getInstance("DES").generateKey();
            StringEncrypter desEncrypter = new StringEncrypter(desKey, desKey.getAlgorithm());
            return desEncrypter.encrypt(cadena);
        } catch (NoSuchAlgorithmException e) {
        }
        return null;
    }

    public static String desencriptarCadena(String s) {
      try {
          Polybius p = new PolybiusImpl();
          SecretKey desKey       = KeyGenerator.getInstance("DES").generateKey();
          StringEncrypter desEncrypter = new StringEncrypter(desKey, desKey.getAlgorithm());
          String desDecrypted       = desEncrypter.decrypt(s);
          return p.desPolybius(desDecrypted);
        } catch (NoSuchAlgorithmException e) {
      }
      return null;
    }
